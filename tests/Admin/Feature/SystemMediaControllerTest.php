...
class SystemMediaControllerTest extends AdminTestCase
{
    ...
    {
        $categoryId = SystemMediaCategory::factory()->create(['folder' => 'tests'])->id;
        ...
        $this->assertTrue($this->storage->exists($path));
        $this->assertDatabaseMissing('system_media', ['id' => $mediaId1]);

        $res = $this->destroyResource($mediaId2);
        $res->assertStatus(204);
        $this->assertNotTrue($this->storage->exists($path));
        $this->assertDatabaseMissing('system_media', ['id' => $mediaId2]);
    }
    ...
    public function testEdit()
    {
        ...
        $res->assertStatus(200)
            ->assertJsonFragment(['id' => $id, 'category_id' => 0]);
    }
    ...
    public function testUpdate()
    {
        ...
        $res = $this->updateResource($mediaId, ['category_id' => 999]);
        $res->assertJsonValidationErrors(['category_id']);

        $res = $this->updateResource($mediaId, ['category_id' => $categoryId]);
        $res->assertStatus(201);

        $this->assertDatabaseHas('system_media', ['id' => $mediaId, 'category_id' => $categoryId]);
    }
    ...
    public function testBatchUpdate()
    {
        ...
        $res = $this->put(route('admin.system-media.batch.update'), ['category_id' => $categoryId, 'id' => $mediaIds]);
        $res->assertStatus(201);

        $this->assertDatabaseHas('system_media', ['id' => $mediaIds[0], 'category_id' => $categoryId]);
        $this->assertDatabaseHas('system_media', ['id' => $mediaIds[1], 'category_id' => $categoryId]);
    }
    ...
    public function testBatchDestroy()
    {
        ...
        $res = $this->delete(route('admin.system-media.batch.destroy'), ['id' => [1, 2]]);
        $res->assertStatus(204);
