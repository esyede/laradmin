<template>
  <page-content>
    <space class="my-1">
      ...
      <a-button @click="createDialog = true">Add</a-button>
      <cache-config/>
    </space>
    ...
    >
      ...
      <a-table-column title="Actions" :width="200">
        <template #default="record">
          <space>
            <router-link :to="`/configs/create?category_id=${record.id}`">Create</router-link>
            <router-link :to="`/configs?category_id=${record.id}`">Edit</router-link>
